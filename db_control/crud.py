...
import json
import pandas as pd
#from db_control.connect import engine ----> Azureとの接続のためコメントアウト
from db_control.connect_MySQL import engine
#from db_control.mymodels import Customers----> Azureとの接続のためコメントアウト
...
def myupdate(mymodel, values):
    ...
    try:
        ...
        with session.begin():
            #result = session.execute(query)
            obj = session.query(mymodel).filter(mymodel.customer_id == customer_id).first()
            if obj:
